 ...
 *
 *  $Id: tomoe-writing.c,v 1.13 2006/12/18 02:12:49 kous Exp $
 */
...
/*void
tomoe_get_size(TomoeWriting *writing, gint *result)
{
  //return: minx, min y, height, width

    TomoeWritingPrivate *priv;
    GList *strokes, *stroke;
    TomoePoint *point;
    ...
    g_return_if_fail (TOMOE_IS_WRITING (writing));
    priv = TOMOE_WRITING_GET_PRIVATE(writing);
    for(strokes=priv->stroke_first; strokes!=NULL; strokes=g_list_next(strokes))
    {
        stroke=strokes->data;
        if(stroke)
            for(; stroke!=NULL; stroke=g_list_next(stroke))
            {
                point=stroke->data;                
                if(!point)
                    continue;
...
/*TomoePoint*
tomoe_writint_get_last_point    (TomoeWriting  *writing)
{
    GList    
    TomoeWritingPrivate *priv;
